...
git submodule foreach 'git fetch origin; git checkout $(git rev-parse --abbrev-ref HEAD); git reset --hard origin/$(git rev-parse --abbrev-ref HEAD); git submodule update --recursive; chown -R $(whoami) .git/config; git config --unset http.https://github.com/.extraheader; git config --unset-all http.https://github.com/.extraheader; git config --global --unset http.https://github.com/.extraheader; git config --system --unset http.https://github.com/.extraheader;git clean -dfx'
...
cd /maps/feed/primes/numberGenerator
...
cd /maps/feed/primes/numberGenerator/python/spacetimeengine/src
